...
Portafolio para rol **Data Scientist (E2E / Applied)**: modelado, despliegue (API), dashboards y buenas prácticas.
...
## Perfil
Data Scientist (E2E) enfocado en soluciones completas: preparación de datos, modelado, validación y despliegue (FastAPI), con dashboards (Streamlit) para negocio. Transición desde operación y consultoría técnica (MacDaniels), con fortalezas en diagnóstico, comunicación con clientes y ownership. Portafolio aplicado en forecasting, detección de fraude, riesgo crediticio, churn y analítica de retención.
...
- Subir 1-2 apps (Streamlit/Render) con enlace público
- Incluir dataset de demo reducido o generado
- Agregar instrucciones de despliegue y variables de entorno
- Capturas actualizadas del dashboard en cada README

## Destacados (Top 3)
1) **Fraud Detection System (E2E)** — Detección de fraude con API + dashboard.  
   - Métricas: AUC 0.9528 | Precision 0.9362 | Recall 0.7213  
...
2) **Credit Risk Scoring (E2E)** — Scoring crediticio con calibración y umbral por costos.  
   - Métricas: AUC 0.7813 | KS 0.4251 | Recall 0.8704  
   - Repo: `projects/credit-risk`

3) **Walmart Demand Forecasting (E2E)** — Forecasting retail con pipeline + API/dashboard.  
   - Métricas: MAE 0.6845 | RMSE 3.9554 | MAPE 52.75%  
   - ROI demo: ~$467K/año (10 tiendas)  
   - Repo: `projects/demand-forecasting`

## All Projects (9)
- `projects/fraud-detection` — AUC 0.9528 | Precision 0.9362 | Recall 0.7213
- `projects/credit-risk` — AUC 0.7813 | KS 0.4251 | Recall 0.8704
- `projects/demand-forecasting` — MAE 0.6845 | RMSE 3.9554 | MAPE 52.75%
- `projects/powergrid-forecast` — MAPE 5.10% | R2 0.9555 | MAE 319.88 kW
- `projects/diabetes-prediction` — AUC 0.8414 | Accuracy 0.7857 | Recall 0.7407
- `projects/churn-prediction` — AUC 0.8380 | Recall 0.9251 | Precision 0.4251
- `projects/retail-inventory` — MAE 6,937 | RMSE 24,318 | R2 0.119 (MVP)
- `projects/olist-retention` — AUC 0.995 | Precision 0.997 | Recall 0.964 (post-hoc demo)
- `projects/telco-churn` — AUC 0.832 | F1 0.624 | ROI 4.56x
